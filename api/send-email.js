...
export default async function handler(req, res) {
  ...
  if (!to || !otp) return res.status(400).json({ success: false, message: 'to and otp required' });

  const finalSubject = subject || 'Your Password Reset OTP — ZUNO';
  const finalHtml = html || `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto"><h2>Your OTP</h2><div style="font-size:32px;font-weight:800;letter-spacing:0.3em">${otp}</div><p>Expires in 10 minutes.</p></div>`;
  const finalText = text || `Your ZUNO OTP is: ${otp} (expires 10 min)`;
