...
test('coupon percent discount computed correctly', async () => {
  const r = await couponService.validate('SUPER10', 'shop', 10000, 1);
  assert.equal(r.valid, true);
  assert.equal(r.discount, 1000);
});

test('coupon with minimum order rejects small carts', async () => {
  const r = await couponService.validate('FLAT50', 'shop', 5000, 1);
  assert.equal(r.valid, false);
  assert.match(r.message, /Minimum/i);
});

test('computeTotals adds tax and total', async () => {
  const t = await orderService.computeTotals({ module: 'shop', subtotal: 10000, couponCode: null, userId: 1 });
  assert.equal(t.subtotal, 10000);
  assert.ok(t.tax > 0);
  assert.equal(t.total, t.subtotal - t.discount + t.tax);
});

test('computeTotals applies a valid coupon', async () => {
  const t = await orderService.computeTotals({ module: 'shop', subtotal: 10000, couponCode: 'SUPER10', userId: 1 });
...
test('order.createFromCart enforces server-side pricing', async () => {
  ...
  const created = await orderService.createFromCart({ userId: uid, module: 'shop', addressId: aid, couponCode: null, items: [{ productId: pid, name: 'p', price: 10000, quantity: 1, lineTotal: 10000 }] });
  assert.ok(created.orderId);
  assert.ok(String(created.orderId).length > 0);
